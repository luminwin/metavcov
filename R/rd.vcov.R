rd.vcov <- function(r, nt, nc, st, sc, n_rt = 0, n_rc = 0)
{
  ft <- nt - st
  fc <- nc - sc
  ...
  for (k in 1:K){
    ...
    for (i in 1:colum.number){
      for (j in 1:colum.number)
      {list.corr.st.varcovar[[k]][i, j] <- r[[k]][i, j]*n_rc[[k]][i, j]*sqrt(fc[k, i]*fc[k, j]*sc[k, i]*sc[k, j])/((nc[k, i]*nc[k, j])^2)+r[[k]][i, j]*n_rt[[k]][i, j]*sqrt(ft[k, i]*ft[k, j]*st[k, i]*st[k, j])/((nt[k, i]*nt[k, j])^2)
      }
    }
  }
  ...
  for (k in 1:K) {
    for (i in 1:colum.number){
      rd[k, i] <- (st[k, i]/nt[k, i]) - (sc[k, i]/nc[k, i])
    }}
  ...
  list(list.rd.cov = list.corr.st.varcovar, rd.cov = corr.st.varcovar, rd = rd)
}
