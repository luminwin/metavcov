lgOR.vcov <- function(r, nt, nc, st, sc, n_rt = NA, n_rc = NA){
  ...
  for (k in 1:K){
    ...
    for (i in 1:colum.number){
      for (j in 1:colum.number)
      {tmp <- r[[k]][i, j]*n_rc[[k]][i, j]*sqrt((1/sc[k, i] + 1/fc[k, i])*(1/sc[k, j] + 1/fc[k, j]))/sqrt(nc[k, i]*nc[k, j]) + r[[k]][i, j]*n_rt[[k]][i, j]*sqrt((1/st[k, i] + 1/ft[k, i])*(1/st[k, j] + 1/ft[k, j]))/sqrt(nt[k, i]*nt[k, j])
  ...
  list(list.vcov = list.corr.st.varcovar, matrix.vcov = corr.st.varcovar, ef = as.data.frame(lgOR))
}
